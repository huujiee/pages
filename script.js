(() => {
  ...
  setFixedVh();
  // 小屏下根据视口高度动态增大 Apps 与 About 的间距，避免短视口下视觉交叠
  function adjustSectionGap() {
    try {
      const w = window.innerWidth;
      const vh = window.innerHeight;
      const appsScene = document.getElementById('apps');
      const aboutScene = document.getElementById('about');
      if (!appsScene || !aboutScene) return;
      // 仅在小屏且“高度小于阈值”时启用动态调整；否则恢复默认样式
      const THRESHOLD_H = 880;   // 视口高度阈值（px）— 可按需调整
      const SCALE = 1;         // 调整强度（越大间距增长越快）
      const MAX_EXTRA = 240;     // 最大额外间距（px）
      const BASE = 24;           // 动态期的基础增量（px），非动态期不应用

      if (vh < THRESHOLD_H) {
        let extra = (THRESHOLD_H - vh) * SCALE;
        extra = Math.min(MAX_EXTRA, Math.max(0, extra));
        const gap = Math.round(BASE + extra);
        appsScene.style.paddingBottom = gap + 'px';
        aboutScene.style.paddingTop = Math.round(gap * 0.6) + 'px';
      } else 
      {
        appsScene.style.paddingBottom = '';
        aboutScene.style.paddingTop = '';
      }
    } catch (e) { console.warn('[gap adjust] error', e); }
  }
  adjustSectionGap();
  // 仅在方向变化时刷新，避免滚动过程中的尺寸抖动
  window.addEventListener('orientationchange', () => {
    setFixedVh();
    adjustSectionGap();
    try { if (window.ScrollTrigger) setTimeout(() => ScrollTrigger.refresh(), 120); } catch {}
  }, { passive: true });
  // 当仅改变窗口高度（例如桌面竖着缩放浏览器）时，同步刷新触发区间
  let __vhResizeTid;
  window.addEventListener('resize', () => {
    clearTimeout(__vhResizeTid);
    __vhResizeTid = setTimeout(() => {
      setFixedVh();
      adjustSectionGap();
      try { if (window.ScrollTrigger) ScrollTrigger.refresh(); } catch {}
    }, 150);
  }, { passive: true });
  ...
  // 如果本地存在 GSAP 与 ScrollTrigger，则启用高级动画；否则使用原生方案
  const params = new URLSearchParams(location.search);
  const debug = params.get('debug') === '1';
  // 平滑滚动改为“显式开启”，避免惯性导致多次手势
  const useSmooth = params.get('smooth') === '1';
  // i18n
  const I18N = {
    zh: {
      nav: { home: '首页', products: '应用', about: '关于', contact: '联系', privacy: '隐私', terms: '条款' },
      hero: { title: '让每一天都更有趣', sub: '为所有人打造愉悦体验' },
    ...
    en: {
      ...
      hero: { title: 'Make Every Day Delightful', sub: 'Crafting delight for all.' },
  ...
  if (!reduceMotion && hasGSAP) {
    try {
      ...
      // 平滑滚动（可选）
      let lenis;
      if (useSmooth && typeof Lenis !== 'undefined') {
        lenis = new Lenis({ smoothWheel: true, smoothTouch: false, duration: 1.0 });
        function raf(time) { lenis.raf(time); ScrollTrigger.update(); requestAnimationFrame(raf); }
        requestAnimationFrame(raf);
      }

      // 初始态
      // 初始状态：首屏保持可见以呈现打字效果；子元素仅设置位移/缩放
      gsap.set('#home .content h1, #home .content p', { y: 60, scale: 0.98 });
      gsap.set('#apps .content h2, #apps .content p', { opacity: 0, y: 60, scale: 0.98, letterSpacing: '0.3em' });
      gsap.set('#apps .card', { opacity: 0, y: 40, scale: 0.98 });
      gsap.set('#about .content', { opacity: 1 });
      gsap.set('#about .content h2, #about .content p', { y: 60, scale: 0.98, letterSpacing: '0.3em', opacity: 0 });
      // Contact 初始：与 About 保持一致的字间距/位移
      gsap.set('#contact .content', { opacity: 1 });
      gsap.set('#contact .content h2', { y: 60, scale: 0.98, letterSpacing: '0.3em' });
      ...
      const aboutTl = gsap.timeline({ paused: true })
        ...
        .fromTo('#about .content p',  { y: 60, opacity: 0, letterSpacing: '0.3em' }, { y: 0, opacity: 1, letterSpacing: '0em', ease: 'none', duration: 0.5 }, 0);
      ScrollTrigger.create({
        trigger: '#about',
        start: () => 'top bottom',
        end:   () => '+=420',
      ...
      const contactTl = gsap.timeline({ paused: true })
        ...
        .fromTo('#contact .content p',  { y: 30, opacity: 0, letterSpacing: '0.3em' }, { y: 0, opacity: 1, letterSpacing: '0em', ease: 'none', duration: 0.5 }, 0.05);
      ScrollTrigger.create({
        trigger: '#contact',
        start: 'top 95%',
        end:   'top 25%',
  ...
  try { if (!reduceMotion && !hasGSAP) requestAnimationFrame(animate); } catch(e){ console.warn('[animate] error', e); }

  // 原生模式下的全局光球：基于整体滚动进度分段贝塞尔移动
  (function animateBalls(){
    ...
    function frame(){
      ...
      const vision = document.getElementById('about');
      const rectStart = hero.getBoundingClientRect();
      const rectEnd = vision.getBoundingClientRect();
      const docY = window.scrollY || 0;
      const startY = docY + rectStart.top;
      const endY = docY + rectEnd.bottom - window.innerHeight;
      const p = clamp01(((window.scrollY || 0) - startY) / Math.max(1, endY - startY));
      const sets = [ ['home','apps'], ['apps','about'] ];
      const segLen = sets.length;
      const seg = Math.min(segLen-1, Math.floor(p*segLen));
      const segStart = seg/segLen, segEnd = (seg+1)/segLen;
    ...
    try { if (!reduceMotion && !hasGSAP) requestAnimationFrame(frame); } catch(e){ console.warn('[balls] error', e); }
  })();

  // 已移除整页淡入：初始即为可见，通过位移/缩放/字距实现进入/离场节奏
  ...
  (function typeIntro() {
    ...
    const lang = urlLang || storedLang || docDefault;
  ...
  (function smartLinks(){
    ...
    const docDefault = (document.documentElement.lang || '').toLowerCase().startsWith('zh') ? 'zh' : 'en';
    // 优先站点默认语言，避免刷新时受浏览器语言影响
    const preset = urlLang || storedLang || docDefault || browserLang;
    try {
      applyI18n(preset);
